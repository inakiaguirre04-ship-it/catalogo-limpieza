...
// =========================================
let slideActual = 0;
let trackCarrusel = document.getElementById('track-carrusel');
let indicadores = document.querySelectorAll('.indicador');
let temporizadorCarrusel;

// Función principal para mover la pista
function mostrarBanner(index) {
    if (!trackCarrusel) return; // Si no hay carrusel, no hace nada
    
    let totalSlides = document.querySelectorAll('.slide-banner').length;
    
    // Si llegamos al final, vuelve al principio y viceversa
    if (index >= totalSlides) slideActual = 0;
    if (index < 0) slideActual = totalSlides - 1;
    
    // Desliza la pista (multiplica el porcentaje por el banner actual)
    trackCarrusel.style.transform = `translateX(-${slideActual * 100}%)`;
    
    // Actualiza las pintitas de abajo
...
// Para las flechas laterales
function moverCarrusel(direccion) {
    slideActual += direccion;
    mostrarBanner(slideActual);
    reiniciarTemporizador(); // Reinicia el reloj para que no salte justo cuando tocaste
}

// Para tocar los puntitos/líneas de abajo
